...
csm_adj_9_1 <- crop(raster('Q:/My Drive/LkWheeler_Sorghum/LkWheeler_Fusarium_Sorghum/CSM/csm_adj_9_1_noDepth_20cm.tif'), sorghum_area)
...
#   samp <- sampleRandom(extract, 500, sp=T, round(2))
...
# 
#IF USING, NORMALIZE BEFORE MERGING
# plots_9_1 <- merge(plots_9_1, var_df, by.x="id", by.y="plots")
...
spplot(plots_9_1, zcol="crr", main=list(label="Canopy Relief Ratio", cex=2))
...
sp_err_yi_9_1 <- errorsarlm(BuAc ~ mean+sd+skew+PlotCRR, data = plots_9_1, listw = nb2listw(w), zero.policy = T)
